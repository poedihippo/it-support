...
function SideMenu({ user }) {
  ...
  useEffect(() => {
    const getMenuUser = async () => {
      ...
      try{
        ...
        const isStringify = JSON.stringify(filterUser[0]?.sunsafe_response);
        const toObj = JSON.parse(filterUser[0]?.sunsafe_response);
  ...
  console.log(dataMenuUser, "check state user");
